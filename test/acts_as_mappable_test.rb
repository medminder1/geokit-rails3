...
class ActsAsMappableTest < GeokitTestCase
  ...
  # We have to use a subquery here because Postgres doesn't support referring to aliases in HAVING clauses
  def scope_having(scope, conditions, table_alias="wrapped_#{scope.table.name}")
    scope.klass.select("*").from(Arel.sql('(' + scope.to_sql + ") AS #{table_alias}")).having(conditions)
  end
  ...
  def count_scope(scope, table_alias="#{scope.table.name}_results")
    if scope.having_values.any?
      ...
      ActiveRecord::Base.connection.select_value(scope.except(:select).select('COUNT(*)').to_sql).to_i
    end
  end

  def test_override_default_units_the_hard_way
    Location.default_units = :kms
    locations = scope_having(Location.geo_scope(:origin => @loc_a), "distance < 3.97")
  ...
  def test_find_with_compound_condition
    locations = scope_having(Location.geo_scope(:origin => @loc_a), "distance < 5 and city = 'Coppell'")
    assert_equal 2, locations.all.size
    assert_equal 2, count_scope(locations)
  end

  def test_find_with_secure_compound_condition
    locations = scope_having(Location.geo_scope(:origin => @loc_a), ["distance < ? and city = ?", 5, 'Coppell'])
    assert_equal 2, locations.all.size
    assert_equal 2, count_scope(locations)
  end
  ...
  def test_scoped_find_with_distance_condition
    locations = scope_having(@starbucks.locations.geo_scope(:origin => @loc_a), "distance < 3.97")
  ...
  def test_scoped_find_with_compound_condition
    locations = scope_having(@starbucks.locations.geo_scope(:origin => @loc_a), "distance < 5 and city = 'Coppell'")
    assert_equal 2, locations.all.size
    assert_equal 2, count_scope(locations)
  end
  ...
  def test_ip_geocoded_find_with_distance_condition
    GeoKit::Geocoders::MultiGeocoder.expects(:geocode).with(LOCATION_A_IP).returns(@location_a)
    locations = scope_having(Location.geo_scope(:origin => LOCATION_A_IP), "distance < 3.97")
    assert_equal 5, locations.all.size
    assert_equal 5, count_scope(locations)
  end
  ...
  def test_ip_geocoded_find_with_compound_condition
    GeoKit::Geocoders::MultiGeocoder.expects(:geocode).with(LOCATION_A_IP).returns(@location_a)
    locations = scope_having(Location.geo_scope(:origin => LOCATION_A_IP), "distance < 5 and city = 'Coppell'")
    assert_equal 2, locations.all.size
    assert_equal 2, count_scope(locations)
  end
  ...
  def test_ip_geocoded_find_with_secure_compound_condition
    GeoKit::Geocoders::MultiGeocoder.expects(:geocode).with(LOCATION_A_IP).returns(@location_a)
    locations = scope_having(Location.geo_scope(:origin => LOCATION_A_IP), ["distance < ? and city = ?", 5, 'Coppell'])
    assert_equal 2, locations.all.size
    assert_equal 2, count_scope(locations)
  end
  ...
  def test_address_geocode
    GeoKit::Geocoders::MultiGeocoder.expects(:geocode).with('Irving, TX').returns(@location_a)
    locations = scope_having(Location.geo_scope(:origin => 'Irving, TX'), ["distance < ? and city = ?", 5, 'Coppell'])
    assert_equal 2, locations.all.size
    assert_equal 2, count_scope(locations)
  end

  def test_find_with_custom_distance_condition
    locations = scope_having(CustomLocation.geo_scope(:origin => @loc_a), "dist < 3.97")
    assert_equal 5, locations.all.size
    assert_equal 5, count_scope(locations)
  end
  ...
  #  locations = scope_having(CustomLocation.geo_scope(:origin => @custom_loc_a), "dist < 3.97")
  #  assert_equal 5, locations.all.size
  #  locations = scope_having(CustomLocation.count(:origin => @custom_loc_a), "dist < 3.97")
  #  assert_equal 5, count_scope(locations)
  #end
  ...
  def test_find_with_compound_condition_with_custom
    locations = scope_having(CustomLocation.geo_scope(:origin => @loc_a), "dist < 5 and city = 'Coppell'")
    assert_equal 1, locations.all.size
    assert_equal 1, count_scope(locations)
  end

  def test_find_with_secure_compound_condition_with_custom
    locations = scope_having(CustomLocation.geo_scope(:origin => @loc_a), ["dist < ? and city = ?", 5, 'Coppell'])
    assert_equal 1, locations.all.size
    assert_equal 1, count_scope(locations)
  end
  ...
  def test_find_with_array_origin
    locations = scope_having(Location.geo_scope(:origin => [@loc_a.lat,@loc_a.lng]), "distance < 3.97")
    assert_equal 5, locations.all.size
    assert_equal 5, count_scope(locations)
  end
  ...
  def test_find_with_through
    ...
    organizations = scope_having(MockOrganization.geo_scope(:origin => @location_a), "distance < 3.97")
